<template>
    <div class="msgul">
      <ul class="clearfloat son_ul">
        ...
        <li class="fl son_li span3">密码</li>
        <li class="fl son_li span4">手机号</li>
        <li class="fl son_li span5">邮箱号</li>
        <li class="fl son_li span6">注册时间</li>
      </ul>
      <ul class="ful">
        <li v-for="item in personal" :key="item.id" class="fli" v-bind:class="{opbg:item.id%2==0}">
          ...
          <span class="span3">{{item.pass}}</span>
          <span class="span4">{{item.phone}}</span>
          <span class="span5">{{item.email}}</span>
          <span class="span6">{{item.register_time}}</span>
        </li>
      </ul>
      <!--按钮-->
...
export default {
  ...
  created: function () {
    this.$http.get('../../static/data/web_api_1.json', {emulateJSON: true}).then(
      function (response) {
        // 获取总页数
